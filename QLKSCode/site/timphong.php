	...
	<div class="content">
		<div class="login-box">
			<form class="login-form" method="POST">
				<div>Mã phòng: <input type="text" name="maphong" placeholder="VD: 101"></div>
				<div>Kiểu phòng: 
		...
		<?php
			...
			if ($maphong == '')
				$sql1 = "select * from phong where trangthai = 1";
			else
				$sql1 = "select * from phong where map = '$maphong' and trangthai = 1";
			...
			if (isset($_POST['duyet'])) {
				$sql = "(select * from phong where trangthai = 1) INTERSECT ($sql1) INTERSECT ($sql2) INTERSECT ($sql3) order by map asc";
				$result = pg_query($db, $sql);
		    	if(!$result)
		    		echo "Error: Không có phòng nào\n";
			}else{
				$sql = "select * from phong where trangthai = 1 order by map asc";
				$result = pg_query($db, $sql);
		    	if(!$result)
		    		echo "Error: Không có phòng nào\n";
			}
		...
		?>
		<div>
			<center><h2 class="ten">Danh sách phòng</h2></center>
